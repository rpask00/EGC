...
export class ImageService {
  ...
  getLabels() {
    return this.db.object('/photo').valueChanges().pipe(
      map((ob: any) => {
        let map: string[] = []
        let indexArr: number[] = []
        for (let key in ob) {
          let index = 0;
          for (let img in ob[key]) {
            index++
          }
          indexArr.push(index)
          map.push(key)
        }
        return map.filter(key => (key.slice(0, 2) == 'CS') || key.slice(0, 2) == 'LO').map((key, i) => { return { key, quantity: indexArr[i] } }) as { key: string, quantity: number }[]
      })
    )
  }

  getPhotos(folderName: string): Observable<string[]> {
    return this.db.list('/photo/' + folderName).valueChanges()
      .pipe(
